...
Given(/^I update the env to staging$/, () => {
  cy.visit('https://'+Cypress.env('STAGING_USER')+':'+Cypress.env('STAGING_PASS')+'@staging.shift.com');
});

And(/^I navigate to staging cars page$/, () => {
  cy.visit('https://'+Cypress.env('STAGING_USER')+':'+Cypress.env('STAGING_PASS')+'@staging.shift.com/cars');
});

And('I navigate to favorites cars page', () => {
  cy.visit('https://'+Cypress.env('STAGING_USER')+':'+Cypress.env('STAGING_PASS')+'@staging.shift.com/favorites');
});

Given('I navigate to sale pending page', () => {
  cy.visit('https://'+Cypress.env('STAGING_USER')+':'+Cypress.env('STAGING_PASS')+'@staging.shift.com/cars?q=sale+pending');
});
